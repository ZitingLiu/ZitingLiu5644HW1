...
for i in range(N):
    discriminant_score[i]=np.log(multivariate_normal.pdf(samples[:,i],mean=m[1],cov=c[1])/
                                    multivariate_normal.pdf(samples[:,i],mean=m[0],cov=c[0]))
...
ax2.set_title('Perr with True Class Q1a')
plt.tight_layout()
plt.savefig('Q1a_Perr.jpg')
plt.show()
