...
#test=wine[y==6]
#print(test)
covs=np.zeros((7,11,11))
for i in range(7):
    covs[i]=np.add(np.cov(wine[y==i].T),5*np.identity(11))
...
for i in range(N):
    ax1.scatter(principalComponents[((decisions==i) & (y==i)),0 ], 
              principalComponents[((decisions==i) & (y==i)),1 ],
              principalComponents[((decisions==i) & (y==i)),2 ],  
              marker='.', c='green', alpha=.6,label="Class"+str(i)+"correct")
    ax1.scatter(principalComponents[ ((~(decisions==i)) & (y==i)),0], 
              principalComponents[ ((~(decisions==i)) & (y==i)),1], 
              principalComponents[ ((~(decisions==i)) & (y==i)),2], 
...
#ax1.set_xlim((-5, 5))
#ax1.set_ylim((-5, 5))
ax1.set_title('Q3a with PCA')
plt.tight_layout()
#plt.legend(loc='upper right', bbox_to_anchor=(1.5, 1.05),
          #ncol=1, fancybox=True, shadow=True,)
plt.savefig('Q3APCA.jpg')
plt.show()